...
class FastMap:
    def __init__(self, graph, K=2, epsilon=1e-3):
        ...
        self.edge_w    = {(u, v): w for u in graph for v, w in graph[u]}
    ...
    def finite_max(self, iterable, key):
        # Return the element with maximal key(value) among those whose key is finite. If none is finite, return None.
    ...
    def get_farthest_pair(self):
        while True:
            ...
            b = self.finite_max(self.nodes, lambda v: dist_a[v])
            if b is None: # a_seed was an isolated vertex — try again
                continue
            dist_b = self.dijkstra(b)
            a = self.finite_max(self.nodes, lambda v: dist_b[v])
